...
#lstm_predictions = model_240.predict(x_train_240)
#lstm_predictions_flat = lstm_predictions.reshape(lstm_predictions.shape[0], -1)
#X_train_combined = np.hstack([x_train_240.reshape(x_train_240.shape[0], -1), lstm_predictions_flat])




# 训练XGBoost模型
#xg_reg = xgb.XGBRegressor(objective='reg:squarederror', colsample_bytree=0.3, learning_rate=0.1,max_depth=5, alpha=10, n_estimators=1)

#xg_reg.fit(X_train_combined, y_train_240)

# 假设你有测试集数据 x_test_96 和 y_test_96
#lstm_predictions_test = model_240.predict(x_test_240)
#lstm_predictions_test_flat = lstm_predictions_test.reshape(lstm_predictions_test.shape[0], -1)

#X_test_combined = np.hstack([x_test_240.reshape(x_test_240.shape[0], -1), lstm_predictions_test_flat])

# 使用XGBoost进行预测
#xgb_predictions_test = xg_reg.predict(X_test_combined)

# 模型融合
#y_pred_240 = (lstm_predictions_test_flat + xgb_predictions_test) / 2
